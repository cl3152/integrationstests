...
mvn verify -DskipITs=false \
  ...
  -Dxml.payload.files=xml/payload.xml \
...
| `xml.payload.files` | `XML_PAYLOAD_FILES` | `xml/payload.xml` | Kommagetrennte Liste von XML-Dateien aus dem Classpath (`src/test/resources`) oder vom Dateisystem |
| `xml.payload` | `XML_PAYLOAD` | _leer_ | Optionaler Inline-XML-Payload; wenn gesetzt, werden `xml.payload.files` ignoriert |
...
## XML-Payloads ablegen

XML-Dateien können direkt unter `src/test/resources` abgelegt werden. Standardmäßig lädt der Test `src/test/resources/xml/payload.xml`. Mehrere zu sendende XMLs können kommagetrennt angegeben werden; sie werden in der angegebenen Reihenfolge mit derselben JMS Correlation ID gesendet:

```bash
mvn verify -DskipITs=false \
  -Dxml.payload.files=xml/create-message.xml,xml/update-message.xml \
  -Ddb.url=jdbc:oracle:thin:@//localhost:1521/FREEPDB1 \
  -Ddb.username=app \
  -Ddb.password=secret
```

In jeder XML-Datei kann `${correlationId}` verwendet werden. Der Platzhalter wird vor dem Versand durch die pro Testlauf generierte Correlation ID ersetzt. Zusätzlich setzt der Test auf jeder JMS-Nachricht die Properties `correlationId`, `contentType=application/xml` und `messageNumber`.
...
* `src/test/resources/xml/payload.xml` ist der standardmäßig geladene Beispiel-Payload.
